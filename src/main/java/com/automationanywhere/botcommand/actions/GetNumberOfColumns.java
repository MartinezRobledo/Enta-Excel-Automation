...
@BotCommand
@CommandPkg(
        label = "Get Number of Columns",
        name = "getNumberOfColumns",
        description = "Returns the number of columns with data in a sheet",
        icon = "excel.svg"
)
public class GetNumberOfColumns {
    ...
    public Value<Double> action(
        ...
        int cols = ExcelHelpers.getLastColumn(sheet);
        varOutput.set((double) cols);
        return new NumberValue((double) cols);
    }
}
